...
export class Mago extends Base implements Movimientos{
    ...
    atacar(ataque:number):void{
        let daño = ataque * this.inteligencia
        if (daño > 55) {
            console.log('Tu '+ this.tipo +' Lanzo una '+this.habilidades[0]+' la cual fue potenciada en base a su Inteligencia y le causo '+daño+' puntos de daño critico al enemigo!');
        }else{
    ...
    evolucionar(): void {
        this.cambiarTipo('Mago Arcano');
        this.habilidades.push('Rayo Arcano');
        this.Vida += 120; this.nivel += 9;
        this.inteligencia += 15;
    }
    cajaMagica():void{
        let nuevaHabilidad = 'Lluvia Arcana';
